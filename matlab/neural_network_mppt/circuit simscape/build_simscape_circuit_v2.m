...
function build_simscape_circuit_v2()
    % Name of the Simulink model to create
    ...
    add_block('powerlib/Electrical Sources/DC Voltage Source', [mdl '/Rectifier'], 'Position', [150 100 170 150]);
...
%% 5. Quadratic Boost Converter Components
    ...
    set_param([mdl '/L1'], 'BranchType', 'RL', 'Resistance', '0.1', 'Inductance', num2str(L1_val));
    
    % Inductor L2
    add_block('powerlib/Elements/Series RLC Branch', [mdl '/L2'], 'Position', [550 70 610 90]);
    set_param([mdl '/L2'], 'BranchType', 'RL', 'Resistance', '0.1', 'Inductance', num2str(L2_val));
    
    % Capacitor Cdc
    add_block('powerlib/Elements/Series RLC Branch', [mdl '/Cdc'], 'Position', [480 150 500 210], 'Orientation', 'down');
    set_param([mdl '/Cdc'], 'BranchType', 'RC', 'Resistance', '0.01', 'Capacitance', num2str(Cdc_val));
    
    % Capacitor Co
    add_block('powerlib/Elements/Series RLC Branch', [mdl '/Co'], 'Position', [750 150 770 210], 'Orientation', 'down');
    set_param([mdl '/Co'], 'BranchType', 'RC', 'Resistance', '0.01', 'Capacitance', num2str(Co_val));
    ...
    add_line(mdl, ph_rect.LConn(1), ph_GND.LConn(1));

    % L1 to D1(anode) and D2(anode)
    add_line(mdl, ph_L1.RConn(1), ph_D1.LConn(1));
    add_line(mdl, ph_L1.RConn(1), ph_D2.LConn(1));
    
    % D1(cathode) to Cdc(+) and L2(LConn)
    add_line(mdl, ph_D1.RConn(1), ph_Cdc.LConn(1));
    add_line(mdl, ph_D1.RConn(1), ph_L2.LConn(1));
    
    % D2(cathode) to Switch(Drain), L2(RConn), D3(anode)
    add_line(mdl, ph_D2.RConn(1), ph_Sw.LConn(1)); % Mosfet Drain is LConn
    add_line(mdl, ph_D2.RConn(1), ph_L2.RConn(1));
    add_line(mdl, ph_D2.RConn(1), ph_D3.LConn(1));
    
    % D3(cathode) to Co(+)
    add_line(mdl, ph_D3.RConn(1), ph_Co.LConn(1));
    ...
    add_line(mdl, ph_rect.LConn(1), get_param([mdl '/V_measure'], 'PortHandles').LConn(2));
    ...
    % Connect V and I through Zero-Order Hold to MPPT Controller to prevent sample rate starvation
    add_block('simulink/Discrete/Zero-Order Hold', [mdl '/ZOH_V'], 'Position', [350 300 370 320]);
    add_block('simulink/Discrete/Zero-Order Hold', [mdl '/ZOH_I'], 'Position', [350 330 370 350]);
            
    add_line(mdl, 'V_measure/1', 'ZOH_V/1', 'autorouting', 'on');
    add_line(mdl, 'I_measure/1', 'ZOH_I/1', 'autorouting', 'on');
    add_line(mdl, 'ZOH_V/1', 'MPPT_Controller/1', 'autorouting', 'on');
    add_line(mdl, 'ZOH_I/1', 'MPPT_Controller/2', 'autorouting', 'on');
    ...
    set_param(mdl, 'MaxStep', '1e-5'); % Ensure it hits the 24kHz PWM frequency
